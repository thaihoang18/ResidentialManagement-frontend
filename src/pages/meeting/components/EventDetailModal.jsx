    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-4 w-full max-w-md shadow-lg" style={{ backgroundColor: '#ffffff', color: '#000000' }}>
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-semibold accent-text">{event.title}</h2>
        ...
        <div className="flex justify-end gap-3">
          <Button variant="outline" onClick={onEdit} className="accent-outline action-btn">Sửa</Button>
          <Button onClick={() => onDelete(event.id)} className="accent-btn action-btn">Xóa</Button>
        </div>
      </div>
    </div>
  );
